...
axios_1.default.defaults.headers.common['Authorization'] = `Bearer ${process.env.TOKEN_DE_SUA_CONTA_MP}`;
...
const envBool = (raw, defaultValue) => {
...
function isInadimplente(dataVencimento) {
    if (!dataVencimento)
        return false;
...
async function getBloqueioCliente(clienteId) {
    const cliente = await prisma.pix_Cliente.findUnique({
        where: { id: clienteId },
        select: { ativo: true, dataVencimento: true, nome: true },
    });
    if (!cliente)
        return { bloqueado: true, motivo: "CLIENTE_NAO_ENCONTRADO", cliente };
    if (!cliente.ativo)
        return { bloqueado: true, motivo: "CLIENTE_INATIVO", cliente };
...
let ultimoAcessoMaquina01 = null;
let valorDoPixMaquina01 = null;
let valordoPixMaquinaBatomEfi01 = null;
...
function setDashboardCache(userId, data) {
    ...
    if (dashboardCache.size <= DASHBOARD_CACHE_MAX_ITEMS)
        return;
    const ordered = [...dashboardCache.entries()].sort((a, b) => a[1].lastAccessAt - b[1].lastAccessAt);
...
async function processarPagamentosPendentes() {
    if (processandoPagamentosPendentes)
        return;
    processandoPagamentosPendentes = true;
    try {
        const limite = new Date(Date.now() - PAGAMENTO_PENDENTE_TIMEOUT_SEGUNDOS * 1000);
        const pendentes = await prisma.pix_Pagamento.findMany({
            where: {
                status: "PENDENTE",
                estornado: false,
                data: { lt: limite },
                mercadoPagoId: { not: null },
                NOT: { mercadoPagoId: "CASH" },
            },
            take: 200,
            orderBy: { data: "asc" },
        });
        for (const pagamento of pendentes) {
            const paymentId = pagamento.mercadoPagoId ? String(pagamento.mercadoPagoId) : "";
            if (!paymentId)
                continue;
            const clienteId = pagamento.clienteId ? String(pagamento.clienteId) : "";
            if (!clienteId)
                continue;
            const cliente = await prisma.pix_Cliente.findUnique({
                where: { id: clienteId },
                select: { mercadoPagoToken: true },
            });
            const tokenCliente = cliente?.mercadoPagoToken ? String(cliente.mercadoPagoToken) : "";
            if (!tokenCliente)
                continue;
            const jaEstornado = await prisma.pix_Pagamento.findFirst({
                where: { id: pagamento.id, estornado: true },
                select: { id: true },
            });
            if (jaEstornado)
                continue;
            ...
            await prisma.pix_Maquina.updateMany({
                where: {
                    id: pagamento.maquinaId,
                    valorDoPix: pagamento.valor,
                },
                data: {
                    valorDoPix: "0",
                },
        ...
        console.error("Erro ao processar pagamentos pendentes:", err);
    }
    finally {
        processandoPagamentosPendentes = false;
    }
}
setInterval(() => {
...
function verifyJWT2(req, res, next) {
    ...
    jsonwebtoken_1.default.verify(token, SECRET_REDEFINICAO, (err, decoded) => {
        if (err) {
            res.status(401).json({
...
function calcularPulsosDinamicos(valorPix, valorPorPulso = 1.0, maquina, metodoPagamento) {
    ...
    if (podeAplicarBonus) {
        const regras = Array.isArray(maquina?.bonusRegras)
            ? maquina.bonusRegras
            : [];
        const valorPagoCentavos = Math.round(Number(valorPix) * 100);
 ...
 */
function tempoOffline(dataUltimoAcesso) {
 ...
 */
async function notificar(urlDiscordWebhook, online, offline) {
    try {
        const embeds = [
            {
 ...
 */
async function notificarDiscord(urlDiscordWebhook, titulo, detalhe) {
    try {
        const dataAtual = new Date();
 ...
 */
function gerarChaveIdempotente(tamanho = 32) {
 ...
 * @returns Dados da resposta do estorno ou undefined em caso de erro
 */
async function estornarMP(id, token, motivoEstorno, tamanhoChave = 32) {
    const url = `https://api.mercadopago.com/v1/payments/${id}/refunds`;
...
app.post("/config", async (req, res) => {
    try {
        ...
        if (p) {
            ...
            req.body.senha = await bcrypt_1.default.hash(senha, salt);
            //req.body.dataInclusao = new Date(Date.now());
            const pessoa = await prisma.pix_Pessoa.create({ data: req.body });
            pessoa.senha = "";
            return res.status(200).json({ msg: "Cadastro efetuado com sucesso! Acesse o painel ADM V4" });
...
app.post("/cliente", verifyJwtPessoa, async (req, res) => {
    try {
        const { nome, email, senha } = req.body;
        if (!nome || !email || !senha) {
            return res.status(400).json({ error: "Campos obrigatórios: nome, email e senha" });
...
app.put("/cliente", verifyJwtPessoa, async (req, res) => {
    try {
        req.body.pessoaId = req.userId;
...
// Funcionário
app.post("/funcionario", verifyJWT, async (req, res) => {
    try {
        console.log(req.body);
        ...
        let funcionario;
        try {
            funcionario = await prisma.funcionario.create({ data: req.body });
        }
        catch (e) {
            if (e?.code === 'P2002' && e?.meta?.target?.includes('cpf')) {
...
app.put("/cliente-sem-token", verifyJwtPessoa, async (req, res) => {
    try {
        req.body.pessoaId = req.userId;
        var clienteAtualizado = await prisma.pix_Cliente.update({
            where: {
...
app.put("/cliente-trocar-senha", verifyJwtPessoa, async (req, res) => {
    var novaSenha = "";
    var senhaCriptografada = "";
    try {
        novaSenha = criarSenha();
        const salt = await bcrypt_1.default.genSalt(10);
        senhaCriptografada = await bcrypt_1.default.hash(novaSenha, salt);
...
app.post("/cliente-trocar-senha-himself", verifyJWT, async (req, res) => {
...
app.post("/maquina", verifyJwtPessoa, async (req, res) => {
    try {
        req.body.pessoaId = req.userId;
...
app.post("/maquina-cliente", verifyJWT, async (req, res) => {
    try {
        req.body.clienteId = req.userId;
...
//   }
// });
app.put("/maquina", verifyJwtPessoa, async (req, res) => {
    try {
...
// });
//DELETAR MÁQUINA ADM
app.delete("/maquina", verifyJwtPessoa, async (req, res) => {
    try {
        if (!req.body.id) {
...
//DELETAR MÁQUINA....
app.delete("/maquina-cliente", verifyJWT, async (req, res) => {
    try {
        if (!req.body.id) {
            return res.status(500).json({ error: `>>:informe o id da máquina que deseja deletar` });
        }
...
app.get("/consultar-maquina/:id", async (req, res) => {
    try {
        ...
        if (maquina) {
            ...
            const pagamentoPendente = await prisma.pix_Pagamento.findFirst({
                where: {
                    maquinaId: maquinaId,
                    estornado: false,
                    status: "PENDENTE",
...
app.post("/credito-remoto", verifyJwtPessoa, async (req, res) => {
    try {
        const maquina = await prisma.pix_Maquina.findUnique({
            where: {
                id: req.body.id,
...
//SIMULA UM CRÉDITO REMOTO
app.post("/credito-remoto-cliente", verifyJWT, async (req, res) => {
    try {
        const maquina = await prisma.pix_Maquina.findUnique({
            where: {
                id: req.body.id,
...
app.post("/login-pessoa", async (req, res) => {
    try {
        ...
        const validPassword = await bcrypt_1.default.compare(req.body.senha, user.senha);
        if (!validPassword) {
            throw new Error('Password or Email Invalid');
        }
        ...
        const token = jsonwebtoken_1.default.sign({ userId: user.id }, SECRET_PESSOA, { expiresIn: 3600 }); //5min = 300 para 1h = 3600
        if (NOTIFICACOES_LOGINS) {
...
app.post("/login-cliente", async (req, res) => {
    try {
        ...
        //explicação top sobre jwt https://www.youtube.com/watch?v=D0gpL8-DVrc
        const token = jsonwebtoken_1.default.sign({ userId: user.id }, SECRET, { expiresIn: 3600 }); //5min = 300 para 1h = 3600
        var warningMsg = "";
        if (user) {
...
app.post("/login-funcionario", async (req, res) => {
    try {
        ...
        return res.json({ cpf: user.cpf, id: user.id, clienteId: user.clienteId, type: "pessoa", key: "FUNCIONARIO", name: user.nome, lastLogin: user.ultimoAcesso, warningMsg: warningMsg, token });
    }
    catch (error) {
        const { message } = error;
        return res.status(403).json({ error: message });
...
app.get("/maquinas", verifyJWT, async (req, res) => {
    const userId = req.query.clienteId && req.query.clienteId != 'undefined' ? req.query.clienteId : req.userId;
    console.log(userId);
    try {
        ...
        const saidasHojeMap = {};
        const parseSaidaQuantidade = (operadora) => {
        ...
        for (const s of saidasHoje) {
            if (!saidasHojeMap[s.maquinaId])
                saidasHojeMap[s.maquinaId] = 0;
...
app.get("/clientes-analise", verifyJwtPessoa, async (req, res) => {
...
app.get("/clientes", verifyJwtPessoa, async (req, res) => {
    ...
    try {
        ...
        await prisma.pix_Cliente.update({
            ...
            data: {
                ativo: true,
                status: null,
                dataVencimento: (0, moment_1.default)(req.body.dataVencimento).toDate()
            }
        });
...
async function verificarRegistroExistente(mercadoPagoId, maquinaId) {
    try {
...
app.post("/webhookmercadopago/:id", async (req, res) => {
    try {
...
app.post("/decrementar-estoque/:id/", async (req, res) => {
    try {
        ...
        let novoEstoque = maquina.estoque !== null ? maquina.estoque - Number(value) : -1;
        // Perform the update
        await prisma.pix_Maquina.update({
            where: {
                id: req.params.id,
...
//RELATORIO DE PAGAMENTOS POR MÁQUINA
app.get("/pagamentos-adm/:maquinaId", verifyJwtPessoa, async (req, res) => {
    console.log(`${req.params.maquinaId} acessou a rota de pagamentos.`);
    try {
...
//RELATORIO DE PAGAMENTOS POR MÁQUINA POR PERÍODO
app.post("/pagamentos-periodo-adm/:maquinaId", verifyJwtPessoa, async (req, res) => {
    try {
        var totalRecebido = 0.0;
        var totalEstornado = 0.0;
        var totalEspecie = 0.0;
...
const parseStringPromise = util.promisify(xml2js_1.default.parseString);
var estornarOperacaoPagSeguroCount = 0;
async function estornarOperacaoPagSeguro(email, token, idOperacao) {
    const url = `https://ws.pagseguro.uol.com.br/v2/transactions/refunds`;
    try {
        const response = await axios_1.default.post('https://ws.pagseguro.uol.com.br/v2/transactions/refunds', null, {
            params: {
                email: email,
                token: token,
...
app.post('/webhookpagbank/:idCliente', async (req, res) => {
    try {
...
app.post('/mp-qrcode-generator/:id/:maquina', async (req, res) => {
    try {
        ...
        const response = await axios_1.default.post(mercadoPagoUrl, pagamentoPix, { headers });
...
app.get('/verificar-pagamento/:idCliente/:idPagamento', async (req, res) => {
    try {
        ...
        const response = await axios_1.default.get(mercadoPagoUrl, { headers });
...
app.post("/monitoramento-adm", verifyJwtPessoa, async (req, res) => {
    try {
...
app.post("/monitoramento-client", verifyJWT, async (req, res) => {
    try {
        const { dataInicio, dataFim, maquinaId } = req.body;
        // Valida se as datas e o ID da máquina foram fornecidos
        if (!dataInicio || !dataFim || !maquinaId) {
...
async function registrarCreditoRemoto(email, ip, idMaquina, valor) {
    try {
...
app.get("/creditos-remotos-adm", verifyJwtPessoa, async (req, res) => {
    try {
...
app.get("/creditos-remotos-client", verifyJWT, async (req, res) => {
    try {
        const { idMaquina } = req.query;
        // Verifica se o idMaquina foi fornecido
        if (!idMaquina) {
...
app.get("/machines-adm", verifyJwtPessoa, async (req, res) => {
    try {
...
app.get("/machines-client", verifyJWT, async (req, res) => {
    try {
        // Busca todas as máquinas e inclui a relação com o cliente
        const maquinas = await prisma.pix_Maquina.findMany({
            where: {
    ...
    try {
        ...
        for (let i = 0; i < numeroDeParcelas; i++) {
            ...
            parcelas.push({
                ...
                status: client_1.StatusPgto.ABERTO, // Corrigir tipo do enum
...
app.get("/pagamento-detalhe/:id", verifyJWT, async (req, res) => {
    try {
        const pagamento = await prisma.pix_Pagamento.findUnique({
            where: { id: req.params.id },
        ...
        const resMp = await axios_1.default.get(url, {
